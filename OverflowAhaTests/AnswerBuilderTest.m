...
{
    STAssertThrows([answerBuilder addAnswersToQuestion:question fromJSON:nil error:nil], @"Not having data should have already been handled");
}
...
-(void)testSendingNonJSONIsAnError
{
    ...
    STAssertFalse([answerBuilder addAnswersToQuestion:question fromJSON:stringIsNotJSON error:&error], @"Can't create answers without real data");
...
{
    STAssertNoThrow([answerBuilder addAnswersToQuestion:question fromJSON:stringIsNotJSON error:nil], @"AnswerBuilder should handle a NULL pointer gracefully");
}

-(void)testSendingJSONWithIncorrectKeysIsAnError
{
    NSError *error = nil;
    STAssertNoThrow([answerBuilder addAnswersToQuestion:question fromJSON:noAnswersJSONString error:&error], @"There must be a collection of answers in the input data");
}


- (void)testAddingRealAnswerJSONIsNotAnError {
    STAssertNoThrow([answerBuilder addAnswersToQuestion: question fromJSON: realAnswerJSON error: NULL], @"Should be OK to actually want to add answers");
}
...
-(void)testAnswerIsProvidedByExpectedPerson
{
    ...
    STAssertEqualObjects(answerer.avatarUrlStr , @"http://www.gravatar.com/avatar/d96ae876eac0075727243a10fab823b3", @"The provided email hash was converted to an avatar URL");}
